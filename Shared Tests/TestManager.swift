...
class TestManager: XCTestCase {
    ...
    func testAppendObject() {
        ...
        switch manager.append(user, to: userStorageIdentifier, using: WrapEncoder<User>()) {
        case .success(let appendResult):
            XCTAssertTrue(appendResult.name == user.name)

            switch manager.all(from: userStorageIdentifier, using: UnboxDecoder<User>()) {
    ...
    func testAppendList() {
        ...
        switch manager.append(all: user, to: userStorageIdentifier, using: WrapEncoder<User>()) {
        case .success(let appendResult):
    ...
    func testAll() {
        ...
        let manager = createManager()
        let user = [User(name: "first user"), User(name: "second user")]
        manager.append(all: user, to: userStorageIdentifier, using: WrapEncoder<User>())
    ...
    func testFirst() {
        ...
        switch manager.first(from: userStorageIdentifier, using: UnboxDecoder<User>()) {
        ...
        switch manager.first(from: userStorageIdentifier, using: UnboxDecoder<User>(), where: { $0.name == user[1].name }) {
        case .success(let first?):
    ...
    func testFilter() {
        ...
        switch manager.filter(from: userStorageIdentifier, using: UnboxDecoder<User>(), including: { $0.name == "Hans" }) {
    ...
    func testContains() {
        ...
        switch manager.contains(userStorageIdentifier, using: UnboxDecoder<User>(), where: { $0.name == "Igor" }) {
        ...
        switch manager.contains(userStorageIdentifier, using: UnboxDecoder<User>(), where: { $0.name == "Wurst" }) {
        case .success(let contained):
    ...
    func testLast() {
        ...
        switch manager.last(from: userStorageIdentifier, using: UnboxDecoder<User>()) {
    ...
    func testRemove() {
        ...
        let result = manager.remove(from: userStorageIdentifier,
                                    using: UnboxDecoder<User>(),
                                    encoder: WrapEncoder<User>(),
    ...
    func testReplace() {
        ...
        manager.append(all: originalList, to: userStorageIdentifier, using: WrapEncoder<User>())

        let replaceList = [User(name: "Lorem"), User(name: "Ipsum"), User(name: "Wurst"), User(name: "Rolph")]

        switch manager.replaceAll(in: userStorageIdentifier, with: replaceList, using: WrapEncoder<User>()) {
